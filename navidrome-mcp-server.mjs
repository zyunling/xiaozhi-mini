...
const TOOLS = {
  ...
  navidrome_get_stream_url: {
    ...
    description: '【音乐播放-必须配合navidrome_search使用】获取歌曲的播放URL。当 navidrome_search 返回歌曲列表后，选择第一首或用户指定的歌曲，用其ID调用本工具。返回内容包含可直接播放的HTTP流地址，必须用 self.online_music.play_music 播放。',
    inputSchema: {
      type: 'object',
      properties: {
        song_id: { type: 'string', description: '歌曲ID（从 navidrome_search 返回结果中获取）' },
        song_name: { type: 'string', description: '歌曲名（从 navidrome_search 返回结果中获取）' },
        artist: { type: 'string', description: '歌手名（从 navidrome_search 返回结果中获取）' },
      },
      required: ['song_id'],
    },
    async handler(params) {
      ...
      // 引导 AI 用 play_type:url + stream URL
      // 测试目的：确认固件 play_type:url 是否会去拉流
